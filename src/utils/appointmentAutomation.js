...
export const sendEmailReminder = async (patientEmail, patientName, date, time, clinicName) => {
    ...
    const html = `
        <div style="font-family: sans-serif; color: #241F1B; max-width: 600px; margin: auto; border: 1px solid #D9D2C7; border-radius: 24px; padding: 40px;">
            ...
            <div style="background-color: #FBFAF8; padding: 20px; border-radius: 16px; border: 1px solid #D9D2C7; margin: 20px 0;">
                <p style="margin: 5px 0;">📅 <b>Fecha:</b> ${date}</p>
                <p style="margin: 5px 0;">⏰ <b>Hora:</b> ${time}</p>
            </div>
            <p style="font-size: 14px; color: #5E554E;">Si necesitas reagendar, por favor contáctanos con anticipación.</p>
            <hr style="border: 0; border-top: 1px solid #D9D2C7; margin: 30px 0;" />
            <p style="font-size: 10px; color: #5E554E; text-align: center;">Enviado automáticamente por ShiningCloud Dental</p>
        </div>
    `;
