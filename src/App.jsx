...
export default function App() {
  ...
  // --- VOZ A TEXTO V63 (CONTINUOUS CON DIAGNÓSTICO) ---
    ...
    if (isListening) {
      ...
      recognition.onerror = (event) => {
          ...
          if (event.error === 'not-allowed') {
              setVoiceStatus('❌ Permiso denegado.');
              alert("Permiso denegado. Habilita el micrófono en la barra de dirección.");
  ...
  return (
    <div className={`min-h-screen flex ${t.bg} ${t.text} transition-all font-sans`}>
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-10 h-screen overflow-y-auto">
        ...
        {/* --- CONTENIDO UNIFICADO DE FICHA (V66) --- */}
        {activeTab === 'ficha' && selectedPatientId && (
            <div className="space-y-4 animate-in slide-in-from-right">
                ...
                {patientTab === 'evolution' && <div className="space-y-2">
                    ...
                    <Button theme={themeMode} className="w-full" onClick={()=>{ const p=getPatient(selectedPatientId); const n={id:Date.now(), text:newEvolution, date:new Date().toLocaleDateString()}; savePatientData(selectedPatientId, {...p, clinical: {...p.clinical, evolution: [n, ...(p.clinical.evolution||[])]}}); setNewEvolution(''); }}>GUARDAR</Button>
                    {getPatient(selectedPatientId).clinical.evolution?.map(ev=>(<Card key={ev.id} theme={themeMode} className="py-3 text-xs"><div className="flex justify-between font-bold text-cyan-500 mb-1"><span>{ev.date}</span></div><p>{ev.text}</p></Card>))}
                </div>}
