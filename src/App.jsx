...
export default function App() {
  ...
  // FIX: Función blindada para recuperar pacientes
  const getPatient = (id) => {
      // Estructura base para paciente nuevo o corrupto
      const base = {
          id,
          personal: { legalName: id },
          anamnesis: { recent: '', remote: '' }, // AQUI ESTABA EL ERROR
      ...
      // Merge inteligente: Si existe el paciente pero le falta anamnesis, se la agrego
      return {
          ...base,
          ...existing,
          anamnesis: existing.anamnesis || base.anamnesis,
  ...
  const sendWhatsApp = (phone, text) => {
      ...
      // Asumimos Chile (56) si no trae código
      const finalPhone = cleanPhone.startsWith('56') ? cleanPhone : `56${cleanPhone}`; 
      window.open(`https://wa.me/${finalPhone}?text=${encodeURIComponent(text)}`, '_blank');
  };

  // Helper para obtener teléfono desde el nombre (ya que Appointments solo guarda nombre por ahora)
  const getPatientPhone = (name) => {
      if (!name) return '';
      // Buscamos en el objeto patientRecords
  ...
  return (
    <div className={`min-h-screen flex ${t.bg} ${t.text} transition-all font-sans`}>
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-10 h-screen overflow-y-auto">
        ...
        {/* --- RESTO DE TABS (CLINICAL - RECETAS) --- */}
