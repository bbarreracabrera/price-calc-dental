...
import { 
  ...
  ChevronLeft, ChevronRight, Users, Clock, DollarSign, PenTool, FileSignature, Edit3, Loader, TrendingDown, CreditCard, Banknote, Box, Minus, AlertTriangle, Shield, Mic, MicOff
} from 'lucide-react';
...
const PatientSelect = ({ theme, patients, onSelect, placeholder = "Buscar Paciente..." }) => {
    ...
    return (<div className="relative w-full z-20"><InputField theme={theme} icon={Search} placeholder={placeholder} value={query} onChange={e => { setQuery(e.target.value); setShowResults(true); }} onFocus={() => setShowResults(true)} />{showResults && query && (<div className={`absolute left-0 right-0 top-full mt-2 rounded-xl border max-h-48 overflow-y-auto shadow-xl ${t.card}`}>{results.length > 0 ? results.map(p => (<div key={p.id} onClick={() => { onSelect(p); setQuery(p.personal.legalName); setShowResults(false); }} className="p-3 hover:bg-white/5 cursor-pointer border-b border-white/5 last:border-0"><p className="font-bold text-sm">{p.personal.legalName}</p></div>)) : (<div className="p-3 text-xs opacity-50">No encontrado. <span className="underline cursor-pointer font-bold" onClick={()=>{onSelect({id:'new', name: query}); setShowResults(false);}}>Crear "{query}"</span></div>)}</div>)}</div>);
};
...
export default function App() {
  ...
  // --- VOZ A TEXTO (V63 DIAGNOSTIC) ---
  const [isListening, setIsListening] = useState(false);
  const [voiceStatus, setVoiceStatus] = useState(''); // Feedback visual
  ...
  const getPatient = (id) => patientRecords[id] || { id, personal: { legalName: id }, clinical: { teeth: {}, perio: {}, hygiene: {}, evolution: [] }, consents: [], images: [] };
  ...
  // --- LOGICA DE VOZ (V63 DIAGNOSTIC) ---
  const toggleVoice = () => {
    // 1. Verificación de navegador (Chrome/Edge recomendados)
    if (!('webkitSpeechRecognition' in window)) {
        alert("Tu navegador no soporta esta función. Por favor usa Google Chrome.");
        return;
    }
    ...
    if (isListening) {
      ...
      recognition.lang = 'es-CL'; // Forzar Español Chile
      ...
      recognition.onresult = (event) => {
        ...
        if (final) {
            // V63: Agrega punto seguido automático si hay texto previo
            setNewEvolution(prev => (prev ? prev.trim() + '. ' : '') + final.charAt(0).toUpperCase() + final.slice(1));
        }
      };
      ...
      recognition.onerror = (event) => {
          ...
          setIsListening(false);
          // V63: Mensajes de error amigables
          if (event.error === 'not-allowed') {
              setVoiceStatus('❌ Permiso denegado. Revisa la barra de URL.');
              alert("Acceso al micrófono bloqueado. Haz clic en el ícono de candado 🔒 o cámara 📹 en la barra de dirección y selecciona 'Permitir'.");
          } else if (event.error === 'no-speech') {
              setVoiceStatus('❌ No se escuchó nada.');
          } else if (event.error === 'network') {
              setVoiceStatus('❌ Error de red (Google requiere internet).');
          } else {
  ...
  return (
    <div className={`min-h-screen flex ${t.bg} ${t.text} transition-all font-sans`}>
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-10 h-screen overflow-y-auto">
        ...
        {/* ... (Aquí va el resto de tabs como 'quote', 'agenda', 'ficha', 'clinical' que ya estaban y no cambian, solo se ocultan visualmente por el menu) ... */}
        ...
        {/* --- V62: EVOLUTION + VOICE (IA) --- */}
        {activeTab === 'ficha' && selectedPatientId && patientTab === 'evolution' && (
            <div className="space-y-2">
                <div className={`flex items-start p-3 rounded-2xl transition-all ${t.inputBg}`}>
                    <textarea rows="3" placeholder="Escribir evolución..." className={`bg-transparent outline-none w-full font-bold text-sm resize-none ${t.text}`} value={newEvolution} onChange={e=>setNewEvolution(e.target.value)} />
                ...
                <Button theme={themeMode} className="w-full" onClick={()=>{ 
                    const p=getPatient(selectedPatientId); 
                    const n={id:Date.now(), text:newEvolution, date:new Date().toLocaleDateString()}; 
                ...
                {getPatient(selectedPatientId).clinical.evolution?.map(ev=>(
                    <Card key={ev.id} theme={themeMode} className="py-3 text-xs">
                        <div className="flex justify-between font-bold text-cyan-500 mb-1"><span>{ev.date}</span></div>
                        <p>{ev.text}</p>
                    </Card>
                ))}
            </div>
        ...
        {/* --- RESTO DEL CONTENIDO DE LA FICHA --- */}
        {activeTab === 'ficha' && selectedPatientId && patientTab !== 'evolution' && (
            // ... (Resto de pestañas de ficha: personal, anamnesis, clinical, perio, consent, images)
            // NOTA: Tuve que envolver esto en un condicional para no duplicar código
            // Se mantiene la misma lógica de V62 para el resto de tabs
