...
export default function CariogramCalculator({ mode = 'public', patientData, onSave, onClose }) {
    ...
    return (
        <div className="print-area bg-white rounded-3xl border border-[#D9D2C7] p-6 md:p-8">
            ...
            <div className="grid md:grid-cols-2 gap-8 items-start">
                ...
                <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="w-full grid grid-cols-1 gap-1.5">
                        <LegendItem color="#46523C" label="Oportunidad de evitar caries" value={result.chance} />
                        <LegendItem color="#9B7E7A" label="Bacterias"                    value={result.bacteria} />
                        <LegendItem color="#D9A86C" label="Dieta"                        value={result.diet} />
                        <LegendItem color="#8A7F74" label="Susceptibilidad"              value={result.susceptibility} />
                    ...
                    <div className="w-full bg-[#FBFAF8] rounded-2xl p-4 border border-[#D9D2C7] text-center">
                        ...
                        <p className="text-xs text-[#241F1B] font-bold">{riskInfo.recommendation}</p>
                    </div>
                ...
                <div className="mt-6 space-y-4">
                    {mode === 'public' && (
                        <div className="w-full bg-[#FFF8F8] border-l-4 border-[#B92323] rounded-r-2xl p-3">
                            ...
                            <p className="text-xs text-[#241F1B] leading-relaxed">
                    ...
                    <div className="w-full bg-white border border-[#D9D2C7] rounded-2xl p-4">
                        <p className="text-[11px] uppercase tracking-widest text-[#5E554E] mb-3 font-bold">
                            Recomendaciones clínicas
                        </p>
                        <div className="space-y-2.5">
                            {recommendations.map((rec, i) => {
                                const color = { critical: '#B92323', high: '#D9A86C', medium: '#46523C', low: '#5E554E' }[rec.priority];
                                return (
                                    <div key={i} className="border-l-4 pl-3 py-0.5" style={{ borderLeftColor: color }}>
                                        <p className="text-[11px] uppercase tracking-widest font-bold" style={{ color }}>{rec.category}</p>
                                        <p className="text-xs text-[#241F1B] leading-relaxed">{rec.text}</p>
                                    </div>
                                );
                            })}
                        </div>
            ...
            <div className="mt-5 bg-[#FBFAF8] border-l-4 border-[#8A7F74] rounded-r-2xl p-3">
                <p className="text-xs text-[#5E554E]">
...
function PieChart({ result }) {
    ...
    const sectors = [
        { value: result.chance,         color: '#46523C', label: 'Oportunidad', emphasized: true },
        { value: result.bacteria,       color: '#9B7E7A', label: 'Bacterias' },
        { value: result.diet,           color: '#D9A86C', label: 'Dieta' },
        { value: result.susceptibility, color: '#8A7F74', label: 'Susceptibilidad' },
    ...
    return (
        <svg viewBox="0 0 300 300" style={{ width: '100%', maxWidth: 320 }} aria-label="Cariogram — sectores de riesgo">
            ...
            {/* Círculo central */}
            <circle cx={CX} cy={CY} r="58" fill="#FBFAF8" stroke="#D9D2C7" strokeWidth="2" />
            <text x={CX} y={CY - 10} textAnchor="middle" fontSize="34" fontWeight="900" fill="#46523C">
                {result.chance}%
            </text>
            <text x={CX} y={CY + 14} textAnchor="middle" fontSize="9" fill="#5E554E" fontWeight="600" letterSpacing="1.5">
                OPORTUNIDAD
            </text>
        </svg>
    );
...
function LegendItem({ color, label, value }) {
    return (
        <div className="flex items-center justify-between gap-2 px-3 py-2 bg-[#FBFAF8] rounded-xl">
            <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-sm shrink-0" style={{ backgroundColor: color }} />
                <span className="text-xs text-[#241F1B]">{label}</span>
            </div>
            <span className="text-xs font-bold text-[#241F1B]">{value}%</span>
        </div>
    );
}
...
function FactorRow({ factor, value, onChange }) {
    return (
        <div className="bg-[#FBFAF8] border border-[#D9D2C7] rounded-2xl p-3">
            <div className="flex items-start justify-between gap-3 mb-1">
                <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold uppercase tracking-widest text-[#241F1B] leading-tight">
                        {factor.label}
                    </p>
                    <p className="text-[11px] text-[#5E554E] mt-0.5 leading-snug">
                        {factor.description}
                    </p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                    <button
                        ...
                        disabled={value <= 0}
                        className="w-7 h-7 rounded-lg bg-white border border-[#D9D2C7] text-[#241F1B] font-bold text-sm disabled:opacity-30 hover:bg-[#D9D2C7]/40 transition-colors"
                    >
                        −
                    </button>
                    <span className="w-8 text-center font-black text-[#241F1B] text-sm">{value}</span>
                    <button
            ...
            <p className="text-[11px] text-[#46523C] font-bold">
